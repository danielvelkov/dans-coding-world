import { client, Tag, Post } from '@dans-coding-world/prisma-schema';
 ...
 */
export const attachTagsToPost = async (postId: number, tagIds: number[]): Promise<Post> => {
 return await client.post.update({
  where: { id: postId },
  data: {
    tags: {
      create: tagIds.map(id => ({ tag: {
        connect: {
          id
        }
      } })) 
