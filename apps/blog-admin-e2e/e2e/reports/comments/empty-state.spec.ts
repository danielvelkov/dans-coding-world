...
test.describe('Comment reports page - empty state', () => {
  let users: User[] = [];
  ...
  test('shows message when no table rows', async ({ page }) => {
    ...
    await expect(page.locator('table')).toContainText(/no reports/i);
  });
});
