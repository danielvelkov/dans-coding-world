...
import { TABLE_COLUMNS } from './shared.constants.ts';
...
it('table displays empty message on no posts passed', async () => {
	...
	expect(screen.getByText('No posts yet - Create your first post')).toBeInTheDocument();
});
...
describe('Row details', async () => {
	...
	it(`displays each post's title in "Title" column`, async () => {
		...
		for (let i = 0; i < posts?.length; i++)
			expect(within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Title'))).getByText(posts[i].title));
	});
	...
	it(`displays each post's status in uppercase "Status/Visibility" column`, async () => {
		...
		for (let i = 0; i < posts?.length; i++)
			expect(
				within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Status/Visibility'))).getByText(
					posts[i].status.toUpperCase()
				)
			);
	});
	...
	if value different that PUBLIC`, async () => {
		...
		await render(PostsTable, { props: { posts: postsWithAlternatingVisibility } });
		...
		for (let i = 0; i < postsWithAlternatingVisibility?.length; i++)
			if (postsWithAlternatingVisibility[i].visibility === 'MEMBERS_ONLY')
				...
				expect(
					within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Status/Visibility'))).queryByText(
						'(Members-only)'
					)
	...
	it(`displays each post's published date in DD/MM/YYYY format (if present)`, async () => {
		...
		for (let i = 0; i < posts?.length; i++)
			if (posts[i].publishedAt)
				expect(
					within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Published/Edited Date'))).getByText(
						formatDateTo_DD_MM_YYYY(new Date(posts[i].publishedAt as Date))
					)
				).toBeInTheDocument();
			else
				expect(
					within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Published/Edited Date'))).queryByText(
						formatDateTo_DD_MM_YYYY(new Date(posts[i].publishedAt as Date))
					)
				).not.toBeInTheDocument();
	});
	...
	it(`displays each post's updated date in DD/MM/YYYY format 
		...
		for (let i = 0; i < posts?.length; i++)
			if (new Date(posts[i].createdAt).getTime() !== new Date(posts[i].updatedAt).getTime())
				expect(
					within(getTableDataCell(i, TABLE_COLUMNS.indexOf('Published/Edited Date'))).getByText(
...
function getTableDataCell(rowIndex: number, colIndex: number) {
	...
	if (rowIndex > rows.length)
		throw new Error(`Row index bigger than available rows (${rowIndex} > ${rows.length})`);
	const cells = within(rows[rowIndex]).getAllByRole('cell');
	if (colIndex > cells.length)
		throw new Error(`Col index bigger than available columns (${colIndex} > ${cells.length})`);
